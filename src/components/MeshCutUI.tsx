...
import { useMeshCut, MeshCutParams } from '@/hooks/useMeshCut';
...
export interface MeshCutUIProps {
    ...
    onCancel?: () => void;
    initialParams?: MeshCutParams;
    onParamsChange?: (params: MeshCutParams) => void;
}
...
export default function MeshCutUI({
    ...
    const flowApi = useMeshCut(onComplete, initialParams);
    ...
    const steps = useMemo(() => [
        ...
        { name: 'Mesh Stitch', desc: 'Stitch boundaries of cut meshes.', params: [] },
        {
        ...
        { name: 'SkinWeight Computation', desc: 'Compute skin weights for cut meshes.', params: [] },
    ], [flowApi.params.smoothLayers, flowApi.params.smoothFactor]);


    const handleFinish = useCallback(() => {
        const resultMeshes = flowApi.onFinish();
        if (resultMeshes) {
            onComplete?.(resultMeshes);
        }
    ...
    return (
        <FlowUI
            instructions="Cut a mesh along a plane. Press C to toggle drawing mode, draw a line across the mesh, then press Enter to confirm. Escape to cancel the line."
            steps={steps}
